<?php
/*
Plugin Name: Starfield Site Analytics for WordPress
Plugin URI: http://www.starfieldtech.com/
Description: Add Site Analytics to your WordPress blog. This plugin will contact the Site Analytics servers and get your script. You need a Site Analytics account to use it.
Version: 1.4.4
Author: StarfieldTech.
License: GPL 2
*/

$sa_test_mode = false;

require_once( ABSPATH . 'wp-content/plugins/starfield-site-analytics/sa-get-script.php' );

if ( ! class_exists( 'adminstarfieldsiteanalytics' )) {

	class adminstarfieldsiteanalytics {
		...
		function addConfigPage() {
			...
			if ( function_exists('add_submenu_page')) {
				add_submenu_page('plugins.php', 'Starfield Tech. Site Analytics Configuration', 'Starfield Tech. Site Analytics', 1, basename(__FILE__), array('adminstarfieldsiteanalytics','configPage'));
			}
		}
...
if ( !class_exists( 'filterstarfieldsiteanalytics' )) {

	class filterstarfieldsiteanalytics {
		...
		function addsiteanalyticsScript() {
			...
			echo( "\n\n".'<!-- Site Analytics plugin v'.$version.' for Wordpress 2.6 - 3.2.1 (Begin) -->'."\n" );
			...
			echo( "\n<!-- Site Analytics plugin v".$version." for Wordpress 2.6 - 3.2.1 (End) -->\n\n");
		}
...
$saf = new filterstarfieldsiteanalytics();

// add the menu item to the admin interface
add_action( 'admin_menu', array( 'adminstarfieldsiteanalytics', 'addConfigPage' ));

// add the footer so the javascript is loaded in admin and blog pages.
add_action( 'wp_footer', array( 'filterstarfieldsiteanalytics', 'addsiteanalyticsScript' ));
add_action( 'admin_footer', array( 'filterstarfieldsiteanalytics', 'addsiteanalyticsScript' ));
?>